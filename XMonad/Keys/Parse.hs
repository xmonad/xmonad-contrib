--------------------------------------------------------------------
-- |
-- Module      :  XMonad.Keys.Parse
-- Copyright   :  Brent Yorgey <byorgey@gmail.com>
-- License     :  BSD3-style (see LICENSE)
--
-- Maintainer  :  TODO
--
-- A collection of key parsing utilities
--
-- (See also "XMonad.Util.CustomKeys" in xmonad-contrib.)
--
--------------------------------------------------------------------

module XMonad.Keys.Parse where
...
-- NOTE (DJ) This is the first refactoring of EZConfig. This code used to live
-- in that module. This code now does *both* parsing of keystrokes *and* using
-- this to create SubMaps. I think it's worth considering refactoring X.A.SubMap
-- as well, and reducing the code that lives here to purely parsing key-strings.
...
showStroke KeyStroke { mask = m, sym = s} = keymaskToString m ++ keysymToString s

keymaskToString :: KeyMask -> String
keymaskToString msk = concatMap (++"-") $ filter (not . null)
                $ map pick
                [(mod1Mask, "M1") ,(mod2Mask, "M2") ,(mod3Mask, "M3") ,(mod4Mask, "M4")
                ,(mod5Mask, "M5") ,(controlMask, "C") ,(shiftMask,"S")]
    where pick (m, str) = if m .&. complement msk == 0 then str else ""
...
-- | Read a KeyStroke
readStroke :: XConfig l -> String -> Maybe KeyStroke
readStroke c = listToMaybe . map fst . filter (null . snd) . readP_to_S (parseStroke c)

-- | Parse a KeyStroke (essentially the same as parseKeyCombo)
parseStroke :: XConfig l -> ReadP KeyStroke
parseStroke c = toKeyStroke <$> (parseKeyCombo c)
